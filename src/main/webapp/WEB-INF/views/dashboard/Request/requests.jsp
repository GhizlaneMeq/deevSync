...
<html>
  ...
  <h2 class="text-lg font-medium text-white">PENDING REQUEST</h2>
                              ...
                              <tr>
                                  ...
                                  <td class="px-4 py-4 text-sm font-medium text-gray-700 whitespace-nowrap">
                                       <div class="inline-flex items-center px-3 py-1 rounded-full gap-x-2
                                               ${request.status == 'APPROVED' ? 'text-emerald-500 bg-emerald-800' :
                                                 request.status == 'REJECTED' ? 'text-red-500 bg-red-800' :
                                                 request.status == 'PENDING' ? 'text-yellow-500 bg-yellow-800' :
                                  ...
                                  <td class="px-4 py-4 text-sm text-gray-300 whitespace-nowrap">
                                      <div class="flex items-center gap-x-2">
                                          ...
                                          <div>
                                              <h2 class="text-sm font-medium -white ">${request.user.firstName} ${request.user.lastName}</h2>
                                          </div>
                                      </div>
                                  </td>
                                  <td class="px-4 py-4 text-sm text-gray-300 whitespace-nowrap">${request.task.title}</td>
                                  <td class="px-4 py-4 text-sm text-gray-300 whitespace-nowrap">${request.type}</td>
                                      ...
                                      <div class="flex items-center gap-x-6">
                                          <form action="requests?action=editStatus" method="post" class="flex gap-4  justify-center">
                                              ...
                                              <input type="hidden" name="status" value="APPROVED">
                                                <button id="dropdownDefaultButton" data-dropdown-toggle="dropdown" class="transition-colors duration-200 hover:text-indigo-500 dark:text-gray-300 text-indigo-500 focus:outline-none" type="button">
                                                Accept
                                                </button>
                                                ...
                                                <div id="dropdown" class="z-10 hidden divide-y divide-gray-100 rounded-lg shadow w-44 bg-gray-700">
                                                    <ul class="py-2 text-sm text-gray-700 dark:text-gray-200" aria-labelledby="dropdownDefaultButton">
                                                     <c:forEach var="user" items="${users}">
                                                       <button type="submit" name="userId" value="${user.id}" class="block px-4 py-2 hover:bg-gray-600 hover:text-white">
