...
<div class="p-4 sm:ml-64">
    <div class="grid grid-cols-3 gap-4 mb-4">
        <div class="flex flex-col items-center justify-center h-24 rounded-lg bg-gray-800">
            <dt class="order-last text-md font-medium text-gray-400">Not Started Tasks</dt>
            <dd class="text-md font-extrabold text-yellow-400 md:text-5xl"><fmt:formatNumber value="${notStartPercent}" maxFractionDigits="1"/> %</dd>
        </div>
        <div class="flex flex-col items-center justify-center h-24 rounded-lg bg-gray-800">
            <dt class="order-last text-md font-medium text-gray-400">In Progress Tasks</dt>
            <dd class="text-md font-extrabold text-blue-400 md:text-5xl"><fmt:formatNumber value="${inProgPercent}" maxFractionDigits="1"/> %</dd>
        </div>
        <div class="flex flex-col items-center justify-center h-24 rounded-lg bg-gray-800">
            <dt class="order-last text-md font-medium text-gray-400">Completed Tasks</dt>
            <dd class="text-md font-extrabold text-green-400 md:text-5xl"><fmt:formatNumber value="${compPercent}" maxFractionDigits="1"/> %</dd>
        </div>
    </div>
    <div class="p-4 border-2 border-gray-700 border-dashed rounded-lg">
        ...
        <section class="px-11 m-auto">
            <div class="sm:flex sm:items-center sm:justify-between">
                <h2 class="text-lg font-medium text-gray-300">Tasks</h2>
                                    ...
                                    <tr>
                                        ...
                                        <td class="px-4 py-4 text-sm font-medium whitespace-nowrap">
                                            ...
                                            <c:choose>
                                                <c:when test="${task.status == 'NOT_STARTED'}">
                                                    bg-yellow-100/60 text-yellow-500 dark:bg-yellow-800
                                                </c:when>
                                                <c:when test="${task.status == 'IN_PROGRESS'}">
                                                    bg-blue-100/60 text-blue-500 dark:bg-blue-800
                                                </c:when>
                                                <c:when test="${task.status == 'COMPLETED'}">
                                                    bg-green-100/60 text-green-500 dark:bg-green-800
                                                </c:when>
                                                <c:when test="${task.status == 'CANCELED'}">
                                                    bg-red-100/60 text-red-500 dark:bg-red-800
                                                </c:when>
                                            </c:choose>">
                                        ...
                                        <td class="px-4 py-4 text-sm whitespace-nowrap">
                                            <div class="flex items-center gap-x-6">
                                                <a href="tasks?action=details&id=${task.id}" class="text-gray-400 transition-colors duration-200 dark:hover:text-indigo-500 hover:text-indigo-500 focus:outline-none">
